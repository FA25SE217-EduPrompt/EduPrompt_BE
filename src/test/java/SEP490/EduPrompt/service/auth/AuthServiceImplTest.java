...
class AuthServiceImplTest {
    ...
    void login_InvalidPassword_ThrowsAuthFailedException() {
        ...
        assertEquals("Invalid password", exception.getMessage()); // Updated to match service code
        verify(userAuthRepository, never()).save(any());
        verify(jwtUtil, never()).generateToken(anyString(), anyString());
    }

    @Test
    void login_UserNotFound_ThrowsInvalidCredentialsException() { // Updated to match service code
        // Arrange
        when(userAuthRepository.findByEmail(validLoginRequest.getEmail())).thenReturn(Optional.empty());

        // Act & Assert
        InvalidCredentialsException exception = assertThrows(InvalidCredentialsException.class, () -> authService.login(validLoginRequest));
        assertEquals("Invalid email", exception.getMessage()); // Updated to match service code
    ...
    void forgotPassword_ValidEmail_SendsResetToken() {
        ...
        doNothing().when(emailService).sendPasswordResetEmail(eq(sampleUserAuth.getEmail()), eq(sampleUser.getLastName()), eq(resetToken), eq(5));
        ...
        verify(userAuthRepository).save(sampleUserAuth);
        verify(emailService).sendPasswordResetEmail(sampleUserAuth.getEmail(), sampleUser.getLastName(), resetToken, 5);
    }
    ...
    void forgotPassword_EmailSendFailure_ThrowsBaseException() {
        ...
        when(userAuthRepository.findByEmail(request.getEmail())).thenReturn(Optional.of(sampleUserAuth));
        when(jwtUtil.generateTokenWithExpiration(sampleUserAuth.getEmail(), 5)).thenReturn("resetToken");
        doThrow(new RuntimeException("Email failed")).when(emailService).sendPasswordResetEmail(anyString(), anyString(), anyString(), anyInt());
